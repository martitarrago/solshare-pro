import { Plus, Sun, Search, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-heading font-bold text-foreground">Comunidades</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl solar-gradient text-white font-medium text-xs hover:opacity-90 transition-opacity shadow-md shadow-primary/20"
        >
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((community, i) => (
            <div key={community.id} className="animate-fade-in" style={{ animationDelay: `${i * 60}ms` }}>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <Sun className="w-10 h-10 text-accent/40 mx-auto mb-3 animate-float" />
